...
class PDF(FPDF):
    ...
    def add_result_section(self, title, data):
        ...
        self.multi_cell(self.epw, 5, self.safe_text(title), border=0, align='L', new_x='LMARGIN', new_y='NEXT')
        
        self.set_font('Arial', '', 9)
        
        if isinstance(data, list):
            if not data:
                self.multi_cell(self.epw, 5, "  - None found.", align='L', new_x='LMARGIN', new_y='NEXT')
            else:
                for item in data:
                    clean_item = self.safe_text(item)
                    self.multi_cell(self.epw, 5, f"  - {clean_item}", align='L', new_x='LMARGIN', new_y='NEXT')
            ...
            if not data:
                ...
                for k, v in data.items():
                    ...
                    self.multi_cell(self.epw, 5, f"  - {clean_k}: {clean_v}", align='L', new_x='LMARGIN', new_y='NEXT')
        
        else:
            self.multi_cell(self.epw, 5, f"  {self.safe_text(data)}", align='L', new_x='LMARGIN', new_y='NEXT')
...
def create_pdf_report(results, domain):
    ...
    if secrets:
        ...
        pdf.multi_cell(pdf.epw, 5, "CRITICAL: Hardcoded Secrets Found!", align='L', new_x='LMARGIN', new_y='NEXT')
        ...
        for secret in secrets:
            ...
            pdf.multi_cell(pdf.epw, 5, f"  [!] Type: {s_type}", align='L', new_x='LMARGIN', new_y='NEXT')
            pdf.multi_cell(pdf.epw, 5, f"      File: {s_file}", align='L', new_x='LMARGIN', new_y='NEXT')
            pdf.multi_cell(pdf.epw, 5, f"      Snippet: {s_snip}", align='L', new_x='LMARGIN', new_y='NEXT')
            pdf.ln(2)
    else:
